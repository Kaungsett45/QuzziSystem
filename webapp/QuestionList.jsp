...
<body>
	...
	<form action="ShowQuestionList" method="post" class="logoutbtn2"><button class="Btnshowlist active">QuestionList</button></form>
	...
	<p class="queslist">Question List :   ${count}</p>
		<tr class="headen">
			<td>Id</td>
...
<c:forEach items="${questionlist}" var="que" varStatus="status">
		<tr class="whitening">
			...
			<td class="title">${que.question_title}</td>
			<td >${que.subject}</td>
			<td class="edit"><a href="EditDelete?action=Edit&id=${que.question_id}">Edit</a></td>
		</tr>
</c:forEach>
